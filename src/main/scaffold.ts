 ...
 * library-wide `references.bib` (`../../.lctrn/references.bib`). The Quarto
 * document (`manuscript.qmd`) is created by `ensureQuartoDocs`
...
- Read \`../../.lctrn/WRITING_RULES.md\` FIRST. Those are the author's library-wide
  ...
  paper's additions on top of them; \`${LEARNED_EDITS_FILE}\` wins over all of it.
...
/**
 * A project's `.claude/settings.json`. Deliberately does NOT pin `model`: an
 * embedded session runs on whatever Claude Code's own default is, so the app
 * never holds a project back on a model that has since been superseded.
 */
